...
def generate_clips():
    ...
    for i, clip in enumerate(sorted(os.listdir(CLIP_FOLDER))):
        ...
        clips.append(
            dbc.Row([
                dbc.Col(html.Audio(src=f'http://192.168.0.128:8080/{clip}', controls=True), width={"size": 2, "offset": 4}),
...
APP.layout = html.Div([
    dbc.Row(children=[
        dbc.Col(html.H1(children='Review doorbells'), width={"size": 4, "offset": 4})
    ]),
